export default function TermsPage() {
  return (
    <main className="pbPageLegal">

      <div className="pbContainer pbLegalWrapper">

          <h1>Terms of Use _ <span>  AVE® Cosmetics Companies</span></h1>

          <p>
            Please read these Terms of Use carefully before accessing or using
            avecosmetico.com By visiting, browsing, or interacting with this platform,
            you acknowledge that you have read, understood, and agree to be legally
            bound by these Terms. If you do not agree with any part of these Terms,
            you must immediately discontinue use of the platform.
          </p>

          <p>
            AVE® Cosmetics Company is an independent digital platform operated by a Mary Kay
            Independent Beauty Consultant. The platform is designed to provide curated
            product discovery, informational content, and access to external product
            destinations. AVE® Cosmetics Company and avecosmetico.com does not sell products, process payments,
            handle transactions, ship merchandise, or manage order fulfillment.
          </p>

          <p>
            avecosmetics.com platform is not owned, operated, affiliated with, or endorsed by
            Mary Kay Inc. Any references to Mary Kay®, including product names,
            trademarks, or branding elements, are used strictly for identification
            and informational purposes. All trademarks, service marks, and brand assets
            remain the exclusive property of their respective owners.
          </p>

          <p>
            When interacting with product listings, category links, or call-to-action
            elements on this platform, you may be redirected to third-party platforms,
            including official Mary Kay product destinations. All transactions,
            payments, pricing, product availability, shipping, returns, and customer
            support are handled exclusively by those third-party platforms.
          </p>

          <p>
            By continuing to use avecosmetico.com, you acknowledge and agree that any
            purchase or transaction you may complete will occur directly with a
            third-party platform and will be subject to their respective terms,
            policies, and legal framework.
          </p>

                {/* ------------------ */}
          <h2>1. About This Platform</h2>

          <p>
            avecosmetico.com is a digital platform focused on beauty, skincare,
            fragrance, and cosmetic product discovery. The platform is designed
            to provide curated product recommendations, informational content,
            and a simplified user experience for exploring external product
            destinations.
          </p>

          <p>
            avecosmetico.com operates as an independent platform managed by a
            Mary Kay Independent Beauty Consultant. Its primary purpose is to
            present products, highlight categories, and assist users in navigating
            available options through a structured and accessible interface.
          </p>

          <p>
            AVE® cosmetics does not sell products, process payments, handle
            financial transactions, store inventory, ship merchandise, or manage
            order fulfillment. The platform does not act as a retailer, distributor,
            or marketplace.
          </p>

          <p>
            All product-related actions, including pricing, availability, checkout,
            payment processing, shipping, returns, refunds, and customer support,
            are handled exclusively by third-party platforms.
          </p>

          <p>
            When you interact with product listings, category links, or call-to-action
            elements on avecosmetico.com, you may be redirected to third-party
            platforms, including official Mary Kay® product destinations. These
            platforms operate independently and maintain their own systems,
            policies, and legal frameworks.
          </p>

          <p>
            By using avecosmetico.com, you acknowledge that any transaction you may
            complete will occur directly on a third-party platform, and not on
            Vendermarket.com. As such, all contractual relationships, purchase
            agreements, and post-purchase interactions are established solely
            between you and the third-party platform.
          </p>

          <p>
            AVE® Company is not responsible for third-party platform content,
            product accuracy, pricing changes, service availability, transaction
            outcomes, or customer service experiences.
          </p>

          {/* ------------------ */}
          <h2>2. Referral Relationship</h2>

          <p>
            avecosmetico.com operates as a referral-based platform managed by a Mary Kay
            Independent Beauty Consultant. The platform provides access to external
            product destinations through curated links, informational content, and
            structured navigation designed to enhance product discovery.
          </p>

          <p>
            Certain links available on avecosmetico.com may contain referral or tracking
            parameters that allow third-party platforms to attribute user activity to
            the consultant operating this platform. This attribution may be used by
            third-party systems to measure engagement, traffic sources, or completed
            actions such as purchases.
          </p>

          <p>
            avecosmetico.com does not process, store, or manage any referral tracking
            data directly. All tracking technologies, including cookies, identifiers,
            or referral systems, are implemented and controlled exclusively by the
            third-party platforms to which users are redirected.
          </p>

          <p>
            avecosmetico.com does not control and is not responsible for third-party
            systems, including pricing, product availability, promotional offers,
            checkout processes, payment systems, shipping operations, returns,
            or customer service.
          </p>

          <p>
            Any interaction, transaction, or engagement that occurs after leaving
            avecosmetico.com by AVE® Company is governed solely by the terms, policies, and operational
            frameworks of the third-party platform.
          </p>

          <p>
            By using our platform avecosmetico.com, you acknowledge that referral links are provided
            for convenience and informational purposes, and that avecosmetico.com acts
            solely as an intermediary for product discovery and redirection.
          </p>

          {/* ------------------ */}
          <h2>3. Relationship with Mary Kay</h2>

          <p>
            avecosmetico.com is an independent digital platform operated by a Mary Kay
            Independent Beauty Consultant and is not owned, operated, controlled, or
            officially endorsed by Mary Kay Inc. The platform functions independently
            and is designed to provide product discovery and access to external product
            destinations.
          </p>

          <p>
            While avecosmetico.com ou AVE® Cosmetics Company may reference Mary Kay® products, categories, or
            related information, such references are made strictly for identification,
            informational, and product discovery purposes only. AVE® Cosmetics Company does
            not represent itself as an official Mary Kay platform, distributor, or
            corporate entity.
          </p>

          <p>
            All trademarks, service marks, logos, product names, and brand elements
            displayed on this platform remain the exclusive property of Mary Kay Inc.
            or their respective owners. No license, right, or authorization is granted
            to users to use, reproduce, or distribute such materials beyond what is
            permitted under applicable law.
          </p>

          <p>
            AVE® Cosmetics Company or avecosmetico.com does not imply any partnership, joint venture, agency,
            or official relationship with Mary Kay Inc. The presence of product
            references, links, or branding elements should not be interpreted as an
            endorsement, sponsorship, or approval by Mary Kay.
          </p>

          <p>
            Any transactions, communications, or interactions related to Mary Kay
            products occur directly between the user and the official Mary Kay
            platform. Vendermarket.com is not involved in, and assumes no
            responsibility for, any aspect of those interactions.
          </p>

          <p>
            By using avecosmetics.com, you acknowledge and agree that the platform
            operates independently and that any engagement with Mary Kay products
            is subject exclusively to the terms, policies, and operational framework
            of Mary Kay Inc.
          </p>

                  {/* ------------------ */}
          <h2>4. Intellectual Property</h2>

          <p>
            All content available on avecosmetico.com, including but not limited to
            text, layout, structure, design, graphics, user interface elements,
            and original written content, is protected by applicable intellectual
            property laws and remains the property of Vendermarket.com or its
            respective licensors where applicable.
          </p>

          <p>
            Unauthorized use, reproduction, distribution, modification, republication,
            or exploitation of any content from this platform, in whole or in part,
            without prior written authorization is strictly prohibited and may result
            in legal action.
          </p>

          <p>
            avecosmetico.com may display references to third-party products,
            trademarks, brand names, and visual materials, including those related
            to or products including Mary Kay® products. Such elements are used solely for identification,
            informational, and product discovery purposes.
          </p>

          <p>
            All third-party trademarks, logos, product images, and brand elements
            remain the exclusive property of their respective owners, including
            Mary Kay Inc. No ownership, license, or right is claimed or granted
            with respect to such materials beyond their intended use on this platform.
          </p>

          <p>
            Nothing on AVE® Cosmetics Company or avecosmetics.com shall be interpreted as granting, by
            implication or otherwise, any license or right to use any trademark,
            logo, or proprietary material displayed on the platform without the
            prior written consent of the respective owner.
          </p>

          <p>
            Users are prohibited from copying, scraping, redistributing, or
            commercially exploiting any content or data from this platform without
            explicit authorization.
          </p>

                {/* ------------------ */}
          <h2>5. Acceptable Use</h2>

          <p>
            You agree to use Vendermarket.com in a lawful, responsible, and respectful
            manner, and in full compliance with all applicable local, national, and
            international laws and regulations.
          </p>

          <p>
            You must not use this platform in any way that could damage, disable,
            overburden, or impair its functionality, security, or availability,
            or interfere with any other user’s access or experience.
          </p>

          <p>You agree that you will not:</p>

          <ul>
            <li>Use the platform for any unlawful, fraudulent, or deceptive purpose</li>

            <li>
              Attempt to gain unauthorized access to any part of the platform, its
              systems, servers, or associated networks
            </li>

            <li>
              Introduce or distribute viruses, malware, bots, scripts, or any harmful
              or disruptive code
            </li>

            <li>
              Engage in data scraping, data mining, automated extraction, or any
              attempt to collect platform data without authorization
            </li>

            <li>
              Infringe upon intellectual property rights, including copying,
              reproducing, or redistributing platform content without permission
            </li>

            <li>
              Use the platform in a way that could damage its reputation or create
              confusion regarding its purpose or relationship with third-party brands
            </li>

            <li>
              Interfere with the platform’s normal operation, including attempting to
              bypass security measures or overload system resources
            </li>
          </ul>

          <p>
            avecosmetico.com reserves the right to restrict, suspend, or terminate
            access to the platform at any time, without notice, if any violation of
            these Terms is suspected or identified.
          </p>


                  {/* ------------------ */}
          <h2>6. Product Information Disclaimer</h2>

          <p>
            AVE® Cosmetics Company and avecosmetico.com strives to provide accurate, relevant, and up-to-date
            product information, including descriptions, categories, pricing references,
            and general product details. However, avecosmetico.com does not guarantee
            that any information presented on the platform is complete, accurate,
            reliable, current, or free of errors.
          </p>

          <p>
            Product information displayed on avecosmetico.com is provided for
            informational and discovery purposes only. Details such as pricing,
            availability, product specifications, ingredients, promotions, and
            packaging may change at any time without notice.
          </p>

          <p>
            All official product information, including final pricing, availability,
            taxes, shipping details, return policies, and product accuracy, is
            determined exclusively by the third-party platform where the transaction
            occurs, including but not limited to official Mary Kay product
            destinations.
          </p>

          <p>
            avecosmetico.com and AVE® Cosmetics Company does not verify or control third-party product data
            and is not responsible for any discrepancies, inaccuracies, or changes
            that may occur after a user is redirected to an external platform.
          </p>

          <p>
            Users are solely responsible for reviewing and confirming all product
            information directly on the third-party platform before making any
            purchase or decision.
          </p>

          <p>
            AVE® Cosmetics Company and avecosmetico.com shall not be held liable for any loss, damage, or
            misunderstanding resulting from reliance on product information
            presented on this platform.
          </p>


                  {/* ------------------ */}
          <h2>7. Third-Party Platforms</h2>

          <p>
            AVE® Cosmetics Company and avecosmetics.com contains links to third-party platforms, including
            external product destinations such as official Mary Kay® pages.
            These links are provided solely for convenience, informational purposes,
            and to facilitate product discovery.
          </p>

          <p>
            When you click on a link and leave avecosmetico.com, you will be redirected
            to a third-party platform that operates independently. AVE® Cosmetics Company and avecosmetico.com
            does not control, monitor, or manage the content, systems, policies,
            or operations of any third-party platform including Mary Kay.
          </p>

          <p>
            AVE® Cosmetics Company and avecosmetico.com is not responsible for the accuracy, completeness,
            reliability, or availability of any information, products, or services
            provided on third-party platforms.
          </p>

          <p>
            All interactions that occur after redirection, including browsing,
            account creation, transactions, payments, shipping, returns, and
            customer service, are handled exclusively by the third-party platform
            and are subject to their own terms, privacy policies, and legal
            frameworks.
          </p>

          <p>
            AVE® Cosmetics Company and avecosmetico.com does not guarantee the security, functionality,
            or performance of third-party platforms and shall not be held liable
            for any damages, losses, or issues arising from their use.
          </p>

          <p>
            By using AVE® Cosmetics Company platform or avecosmetico.com, you acknowledge and agree that accessing
            third-party platforms is done at your own discretion and risk.
          </p>

          {/* ------------------ */}
          <h2>8. Limitation of Liability</h2>

          <p>
            To the fullest extent permitted by applicable law, AVE® Cosmetics Company and avecosmetico.com,
            its operator, and any associated parties shall not be held liable
            for any direct, indirect, incidental, consequential, special, or
            punitive damages arising out of or related to your use of, or inability
            to use, the platform.
          </p>

          <p>
            This limitation of liability applies to, but is not limited to:
          </p>

          <ul>
            <li>
              Your access to or use of the platform, including any interruption,
              delay, or unavailability of services
            </li>

            <li>
              Any reliance on information, content, product descriptions, or
              recommendations provided on the platform
            </li>

            <li>
              Any transactions, interactions, or engagements conducted on
              third-party platforms after leaving AVE® Cosmetics Company and avecosmetico.com
            </li>

            <li>
              Errors, inaccuracies, or omissions in product information,
              pricing, availability, or content
            </li>

            <li>
              Technical issues, system failures, bugs, or unauthorized access
              affecting the platform or user data
            </li>

            <li>
              Any loss of data, revenue, business opportunity, or expected
              savings resulting from the use of the platform
            </li>
          </ul>

          <p>
            AVE® Cosmetics Company and avecosmetico.com does not guarantee that the platform will operate
            without interruption, error, or security vulnerabilities.
          </p>

          <p>
            In all cases, AVE® Cosmetics Company and avecosmetico.com’s total liability, if any, shall be
            limited to the maximum extent permitted by law.
          </p>

          <p>
            By using AVE® Cosmetics Company and avecosmetico.com, you acknowledge and agree that your use
            of the platform is at your own risk.
          </p>

          {/* ------------------ */}
          <h2>9. Indemnification</h2>

          <p>
            You agree to indemnify, defend, and hold harmless Vendermarket.com,
            its operator, affiliates, partners, and any associated parties from
            and against any and all claims, demands, damages, losses, liabilities,
            costs, or expenses (including reasonable legal fees) arising out of
            or related to your use of the platform.
          </p>

          <p>
            This includes, but is not limited to:
          </p>

          <ul>
            <li>
              Any violation of these Terms of Use or applicable laws and regulations
            </li>

            <li>
              Your misuse of the platform, including unauthorized access, abuse,
              or disruptive behavior
            </li>

            <li>
              Any interaction, transaction, or dispute occurring on third-party
              platforms after leaving AVE® Cosmetics Company and avecosmetico.com
            </li>

            <li>
              Any infringement of intellectual property rights or misuse of
              content obtained from the platform
            </li>

            <li>
              Any claims brought by third parties resulting from your actions,
              conduct, or use of the platform
            </li>
          </ul>

          <p>
            AVE® Cosmetics Company and avecosmetico.com reserves the right, at its own expense, to assume
            exclusive defense and control of any matter otherwise subject to
            indemnification by you, in which case you agree to cooperate fully
            with such defense.
          </p>

          <p>
            This indemnification obligation will survive the termination or
            discontinuation of your use of the platform.
          </p>


          {/* ------------------ */}
          <h2>10. Privacy</h2>

          <p>
            Your use of AVE® Cosmetics Company and avecosmetico.com is also governed by our{" "}
            <a href="/privacy-policy">Privacy Policy</a>, which explains how
            information may be collected, used, and protected when you access
            or interact with this platform.
          </p>

          <p>
            AVE® Cosmetics Company and avecosmetico.com is an independent platform operated by a Mary Kay
            Independent Beauty Consultant and does not process payments or manage
            transactions directly. As a result, certain interactions may involve
            third-party platforms.
          </p>

          <p>
            When you click on links and are redirected to third-party platforms,
            including official Mary Kay® product destinations, your data may be
            collected and processed according to their respective privacy policies
            and terms of use.
          </p>

          <p>
            AVE® Cosmetics Company and avecosmetico.com does not control and is not responsible for the data
            collection, processing, storage, or security practices of any
            third-party platform.
          </p>

          <p>
            For more information, you are encouraged to review the official Mary Kay
            policies:
          </p>

          <ul>
            <li>
              <a
                href="https://www.marykay.com/en-us/pages/legal/privacy-policy"
                target="_blank"
                rel="noopener noreferrer"
              >
                Mary Kay Privacy Policy
              </a>
            </li>
            <li>
              <a
                href="https://www.marykay.com/en-us/pages/legal/terms-of-use"
                target="_blank"
                rel="noopener noreferrer"
              >
                Mary Kay Terms of Use
              </a>
            </li>
          </ul>

          <p>
            By continuing to use AVE® Cosmetics Company and avecosmetico.com, you acknowledge and agree that
            your interactions with third-party platforms are subject to their own
            policies and legal frameworks.
          </p>


          {/* ------------------ */}
          <h2>11. Jurisdiction</h2>

          <p>
            AVE® Cosmetics Company and avecosmetico.com is operated from the United States. By accessing or using
            this platform, you agree that your use is governed by applicable federal
            laws of the United States.
          </p>

          <p>
            Users accessing the platform from outside the United States are responsible
            for ensuring compliance with their local laws and regulations.
          </p>

          <p>
            To the fullest extent permitted by law, any disputes arising out of or
            relating to the use of this platform shall be governed by applicable federal
            laws of the United States. Where federal law does not apply, relevant
            applicable laws may be used as a fallback.
          </p>

          <p>
            You agree to submit to the exclusive jurisdiction of the competent courts
            within the United States for the resolution of any disputes.
          </p>

                {/* ------------------ */}
          <h2>12. Modifications</h2>

          <p>
            AVE® Cosmetics Company and avecosmetico.com reserves the right to update, modify, or revise these
            Terms of Use at any time, at its sole discretion, without prior notice.
          </p>

          <p>
            Any changes or updates will become effective immediately upon being
            posted on this platform, unless otherwise stated.

            It is your responsibility to review these Terms periodically to stay
            informed of any modifications or updates.
            Your continued access to or use of AVE® Cosmetics Company and avecosmetico.com after any changes
            have been posted constitutes your acceptance of the revised Terms.
          </p>

          <p>
            If you do not agree with any updated Terms, you must discontinue use
            of the platform immediately.
          </p>

          {/* ------------------ */}
          <h2>13. Governing Law</h2>

          <p>
            These Terms of Use shall be governed by and construed in accordance
            with the laws of the State of Florida, United States, without regard
            to its conflict of law principles.
          </p>

          <p>
            Any disputes arising out of or relating to these Terms or the use of
            AVE® Cosmetics Company and avecosmetico.com shall be subject to the exclusive jurisdiction of
            the courts located within the State of Florida.
          </p>

        {/* ------------------ */}
        <h2>14. Contact</h2>

        <p>
          For questions regarding these Terms, please contact us through
          the platform’s contact page.
        </p>

      </div>
    </main>
  );
}

